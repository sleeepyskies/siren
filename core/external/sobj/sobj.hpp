...
size_t OBJLoader::calculateIndex(const int index, const IndexType type) const
{
    ...
    case IndexType::POSITION:
        return m_positions.size() - abs(index);
        break;
    case IndexType::NORMAL:
        return m_normals.size() - abs(index);
        break;
    case IndexType::UV:
        return m_textureUVs.size() - abs(index);
        break;
    case IndexType::COLOR:
        return m_colors.size() - abs(index);
        break;
    case IndexType::FACE:
        return m_positions.size() - abs(index);
        break;
...
std::vector<Face> OBJLoader::triangulate(const Face& face) const
{
    ...
    const int n = face.numVertices();
    if (n < 3) {
        throw std::runtime_error("Face must have at least 3 vertices. Line " +
    ...
    for (int i = 1; i < n - 1; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
            f.positionIndices.push_back(face.positionIndices[idx[j]]);
            if (!face.normalIndices.empty()) f.normalIndices.push_back(face.normalIndices[idx[j]]);
            if (!face.colorIndices.empty()) f.colorIndices.push_back(face.colorIndices[idx[j]]);
            if (!face.uvIndices.empty()) f.uvIndices.push_back(face.uvIndices[idx[j]]);
        }
