...
private:
    ...
    struct TableEntry {
        ...
        // @formatter:off
        void kill() { ++generation; api_handle = 0; }
        // @formatter:on
    };
...
public:
    ...
    auto link(const ProxyHandleType proxy_handle, const ApiHandleType api_handle) -> void {
        SIREN_ASSERT(is_valid_id(proxy_handle), "Passed an invalid ProxyHandleType: {}", proxy_handle);
        m_table[proxy_handle.index()].api_handle = api_handle;
    }
