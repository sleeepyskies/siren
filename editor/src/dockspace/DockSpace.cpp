...
DockSpace::DockSpace(const Ref<ecs::Scene>& scene) : m_scene(scene)
{
    // todo: read from some save file how we want to setup UI, but for now just hardcode
    m_panels.emplace_back(makeUref<SceneViewPanel>(m_scene));
    m_panels.emplace_back(makeUref<SceneHierarchyPanel>(m_scene));
}
